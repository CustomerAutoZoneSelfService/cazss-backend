...
public class ResponsesController {
    ...
    /**
     * POST /responses/{id}/response-patterns/ Creates patterns from scratch,
     * assumes no patterns registered
     * keeps untouched response patterns.
     * 
     * @param responseId
     * @return
     */
    ...
    public ResponseEntity<List<CreateResponsePatternDTO>> postPatterns(
        ...
        List<CreateResponsePatternDTO> resultingTree = responsePatternService.addPatterns(id, responsePatterns);
        return new ResponseEntity<>(resultingTree, HttpStatus.CREATED);
    }

    /**
     * PATCH /responses/{id}/response-patterns/ Updates all response patterns keeps
     * untouched response patterns.
     * 
     * @param id
     * @return
     */
    ...
    public ResponseEntity<List<CreateResponsePatternDTO>> patchPatterns(
            @PathVariable Integer id, @RequestBody List<CreateResponsePatternDTO> responsePatterns) {
        List<CreateResponsePatternDTO> requestResponseDTO = responsePatternService.updatePatterns(id, responsePatterns);
        return new ResponseEntity<>(requestResponseDTO, HttpStatus.OK);
    }

    /**
     * /responses/{id}/response-patterns/ Updates all response patterns but assumes
     * untouched response patterns have to be deleted.
     * 
     * @param id
     * @return
     */
    ...
    public ResponseEntity<List<CreateResponsePatternDTO>> putPatterns(
            @PathVariable Integer id, @RequestBody List<CreateResponsePatternDTO> responsePatterns) {
        List<CreateResponsePatternDTO> requestResponseDTO = responsePatternService.replacePatterns(id,
                responsePatterns);
        return new ResponseEntity<>(requestResponseDTO, HttpStatus.OK);
    }
}
