...
/**
 * Initializes default users if they do not already exist. This runner executes once the Spring
...
public class DatabaseInitializer implements ApplicationRunner {
  ...
  /**
   * The callback method executed on application startup. It checks for the existence of an admin,
   * configurator, auditor and user and creates one if not found. Also creates a category if none
   * exists.
   *
  ...
  private void createCategory() {
    if (categoryRepository.count() == 0) {
      CategoryEntity defaultCategory = new CategoryEntity("General", "#F9A8D4");
